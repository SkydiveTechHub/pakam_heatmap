...
export default function Home() {
  ...
  const { isLoaded, loadError } = useLoadScript({
    ...
    libraries: ["places", "visualization"], // Heatmap and Places libraries
  });
    ...
    const fetchData = async () => {
      try {
        const res = await fetch("https://beta.pakam.ng/collector/api/v2/heatmap/transactions?startDate=2025-01-01&endDate=2025-06-30&state=Lagos&minWeight=10&maxWeight=1000");
        if (!res.ok) {
          throw new Error("Network response was not ok");
        }
        const result = await res.json();
        console.log("Heatmap Data:", result.data[0]);
        setData(result.data.map((item: any) => ({ lat: item._id.lat, lng: item._id.long, material: item._id.material, weight: item.quantity, address:item._id.address})));
      } catch (error) {
    ...
    // console.log("color fetched:", colorList);
  ...
  return (
    <main>
      <div>
        {colorList && Object.keys(colorList).length > 0 && (
          <div style={{
            position: 'fixed',
            top: '10px',
            left: '10px',
